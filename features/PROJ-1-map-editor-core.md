...
Subtilität: CSS `mask-image` rasterisiert `<radialGradient>` in Chromium nicht zuverlässig vom SVG-Data-URL — daher gibt es im Preview-Pfad einen neuen Hook `useRasterizedMaskUrl`, der die Glow-Mask via `<img>` + Canvas `drawImage` zu einem PNG-Data-URL bäckt. PNG-Masks rendern überall korrekt. Andere Modi (none/opacity/full) gehen weiterhin direkt als SVG-DataURL durch — kein Overhead. Export-Pfad rasterisiert ohnehin schon implizit über `<Image>` + canvas-`destination-in`.

**2026-04-26 — Poster-Hintergrundfarbe (Darkmode-Toggle):** Neuer Switch im „Außenbereich"-Bereich der Sidebar. Wenn aktiv, übernimmt der Poster-Hintergrund die `background`-Farbe der aktiven Map-Palette (z. B. Navy → `#0b1c33`). So fließen die Glow-Ecken nahtlos in den Poster-BG, statt gegen Weiß zu kontrastieren. Default aus = weiß. Bei `paletteId === 'original'` (kein Palette-Override) bleibt es weiß, da wir die nativen MapTiler-Style-Farben client-side nicht extrahieren. Persistiert via `posterDarkMode: boolean` im Editor-Store; in `useMapExport` zieht's der `buildPosterCanvas`-Initialfill durch, also wirkt's auch in PNG/PDF/Raumansicht.

**2026-04-26 — Floating Vorschau-Button:** Eye-Button oben links auf dem Canvas (Desktop und Mobile, via `PosterCanvas`-Wrapper). Öffnet die Druckvorschau (= bisheriger „Raumansicht"-Modal) aus jedem Sidebar-Tab heraus, statt erst nach Wechsel in den Export-Tab. Tooltip „Vorschau", ARIA „Druckvorschau öffnen". Logik in neuem `PreviewTriggerButton`-Component gekapselt; alter „In Zimmeransicht ansehen"-Button im Export-Tab entfernt. Mobile Layout hat den eigenen Eye-Button (PROJ-18) auch verloren, da `PosterCanvas` ihn jetzt selbst rendert.
