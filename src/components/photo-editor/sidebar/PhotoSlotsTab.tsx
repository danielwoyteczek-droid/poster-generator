...
/**
 * Sidebar-Tab listet alle Letter-Mask-Slots, jeder mit eigenem Upload-Button,
 * Zoom-Slider und Color-Picker (für leere Slots). Re-Use von `uploadPhoto`
 * aus PROJ-19 — keine neue Storage-Logik.
 */
export function PhotoSlotsTab() {
  ...
  if (slots.length === 0) {
    return (
      <div className="p-4">
        <p className="text-sm text-muted-foreground">{t('slotsEmpty')}</p>
      </div>
    )
  }
      ...
      <div className="space-y-3">
        {slots.map((slot, i) => (
          <SlotCard
            ...
            colorOverridden={slot.color !== null}
            isSelected={selectedSlotIndex === i}
            userId={user?.id}
            onSelect={() => setSelectedSlotIndex(i)}
            onPhotoSet={(photo) => setSlotPhoto(i, photo)}
            onCropUpdate={(updates) => updateSlotCrop(i, updates)}
            onColorChange={(color) => setSlotColor(i, color)}
          />
        ))}
      </div>
    </div>
...
function SlotCard({
  ...
  const handleFile = async (file: File) => {
    ...
    try {
      ...
      onSelect()
      toast.success(t('slotPhotoAdded', { index: index + 1 }))
    } catch (err) {
  ...
  return (
    ...
    >
      <div className="flex items-start gap-2">
        <div
          className="w-12 h-12 shrink-0 rounded-sm border border-border flex items-center justify-center text-2xl font-bold leading-none overflow-hidden bg-white"
          style={{ color: photo ? '#94a3b8' : color }}
        >
          {photo ? (
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold">
            {t('slotTitle', { index: index + 1, char })}
          </p>
